...
look for is the prompt that says `user=>`. That shows you can
input Clojure code to evaluate it (turn it into a value) and that
you're currently in the `user` namespace. We need to tell Clojure about our `hello` namespace:
...
If you get a message like these:
 ...
 clojure.lang.Compiler$CompilerException: java.lang.RuntimeException: Unable to resolve symbol: hello in this context, compiling:(/tmp/user164143164117124931.clj:1:1)
